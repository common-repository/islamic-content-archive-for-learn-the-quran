...
class app_ltq_cronjob extends app_ltq_models {
	...
	public function InsetPost() {
		...
		$ltqhtml = new app_ltq_helpers();
		$ltqlang = $ltqhtml -> class_lang;
		
		$posts = $this -> importUrl();
		
		foreach ($posts as $key => $value) {
			
			$parentname = $ltqlang[$key];
...
$opic_ltq_cronjob = new app_ltq_cronjob();
/*$opic_ltq_cronjob->InsetPost();
die;*/
